...
# Title: Tmax percentile (CONUS, tiled, parallel<=12) from local GridMET raws:
#        percentile only (unitless), using 30-year calendar reference.
...
tiles_root   = .abs_path(fs::path(derived_dir, "tmax_metrics"))
conus_root   = .abs_path(fs::path(derived_dir, "conus_drought"))

raw_tmmx_dir = .abs_path(fs::path(interim_dir, "gridmet", "tmmx", "raw"))
...
tmax_timescales_days = function(dates) {
  md      = format(dates, "%m-%d")
...
# ---- Raw TMMX file discovery + date parsing ----------------------------------
.list_raw_tmmx_files = function(dir = raw_tmmx_dir) {
  dir   = .abs_path(dir)
  if (!fs::dir_exists(dir)) stop("Raw TMMX directory not found: ", dir)
  files = fs::dir_ls(dir, regexp = "tmmx_[0-9]{4}\\.nc$", type = "file")
  if (!length(files)) stop("No raw TMMX NetCDF files found in: ", dir)
...
read_tile_values_tmmx_from_files = function(tile_sf, tmmx_files, dates, terra_temp_dir) {
  ...
  rr_list = vector("list", length(tmmx_files))
  for (k in seq_along(tmmx_files)) {
    r_k = try(
      terra::rast(paste0("NETCDF:", as.character(tmmx_files[[k]]), ":", vname)),
      silent = TRUE
    )
    if (inherits(r_k, "try-error")) {
      return(list(vals = NULL, base_r = NULL, msg = paste("read error:", tmmx_files[[k]])))
    }
    rr_list[[k]] = r_k
  }
...
# Safe window mean: tmax is averaged (not summed) over the window
...
write_tmax_metrics_for_tile = function(tile_sf, tmmx_files, dates, periods_days = NULL,
  ...
  io = read_tile_values_tmmx_from_files(
    tile_sf        = tile_sf,
  ...
  rs = compute_tmax_metrics_from_matrix(
    vals         = io$vals,
    dates        = dates,
    base_r       = io$base_r,
...
.mosaic_vrt_to_cog = function(src_files, out_tif) {
  ...
  if (.has_gdal_utils()) {
    vrt_path = fs::path(r_temp, paste0("tmax_", as.integer(stats::runif(1, 1, 1e9)), ".vrt"))
...
run_tmax_metrics = function() {
  ...
  message("Raw TMMX last date: ", last_date_iso)

  # Build tile grid using a single lightweight layer for extent/resolution only
  r_meta = terra::rast(paste0("NETCDF:", as.character(tmmx_files[[1]]), ":air_temperature"))[[1]]
