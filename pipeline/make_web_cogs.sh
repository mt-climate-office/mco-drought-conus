...
for src_tif in "${tif_files[@]}"; do
  ...
  #       cdd-*:      dry-streak length in days, up to the 2557-day window
  #     These use Int32 instead (-21M..21M range). Note scdd-* is NOT here: it is
  #     a z-score clamped to |4.75|, which fits Int16 like spi/spei/eddi do.
  if [[ "$base" == precip-mm_* || "$base" == precip-pon_* || "$base" == precip-dev_* || "$base" == vpd-pon_* || "$base" == cdd-* ]]; then
